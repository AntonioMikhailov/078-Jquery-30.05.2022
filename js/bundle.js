...
function cards() {
  ...
  class MenuCard {
    ...
    changeToRouble() {
      // метод перевода из дол. в рубли
      this.price = this.price * this.transfer;
    }

    render() {
      // создает верстку в виде элементов
      const element = document.createElement('div'); // если доп. классы не передадутся мы сами их добавим
  ...
  const getResource = async url => {
    ...
    if (!res.ok) {
      // если что то не так ( св-во ok или status)
...
function forms() {
  ...
  const forms = document.querySelectorAll('form'); //кнопку в переменную не нужно т.к. form отправляет автоматом если кнликнуть по тегу button
  ...
  const postData = async (url, data) => {
    const res = await fetch(url, {
      // await - значит будет ждать результат запроса и только потом продолжит выполнение
      method: 'POST',
      headers: {
  ...
  `; // но проще добавить класс в CSS
      ...
      form.insertAdjacentElement('afterend', statusMessage); // Создаем метод отправки вариант XML Request - устаревший
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // Применяем метод Fetch
      ...
      .then(data => {
        ...
        showThanksModal(message.success); // После отправки формы очищаем поля и убираем надпись уведомление

        setTimeout(() => {
          statusMessage.remove();
        }, 2000);
      }).catch(() => {
        // сюда можно добавить еще then(), но  мы пропишем блок catch() если будет ошибка от ответа сервера
        showThanksModal(message.failed);
      }).finally(() => {
        // добавляем еще действие очистки формы - независимо от того как ответил сервер
        form.reset();
      });
    });
  } // конец ф. PostData
  ...
  function showThanksModal(message) {
    const prevModalDialog = document.querySelector('.modal__dialog'); // прячем предыдущий контент с полями отправки

    prevModalDialog.classList.add('hide');
    (0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)(); // создаем новый контент в окне
...
function openModal(modalSelector, showModalDelay) {
  ...
  document.body.style.paddingRight = offset + 'px'; // убираем автопоказ если юзер сам кликнул и очищаем показ с задержкой и по скролу
...
function modal(triggetSelector, modalSelector) {
  ...
  const modal = document.querySelector(modalSelector); // const btnCloseModal = document.querySelector('[data-close]');
  ...
  btnOpenModal.forEach(item => {
    item.addEventListener('click', () => openModal(modalSelector)); // или так
...
function slider(_ref) {
  ...
  const totalNum = document.querySelector(totalCounter); // Показываем первый слайд при загрузке
  ...
  function currentIndex(n) {
    showSlide(indexSlide = indexSlide + n); //здесь индекс передастся в остновную ф.showSlide
  }
  ...
  showTotalNum(); // ф. показа текущего номера
  ...
  showCurrentNum(indexSlide); // function showSlide(n) {
  //   offerSlide.forEach(item => { 
  //     item.classList.remove('active');
...
function tabs() {
  ...
  const tabsParent = document.querySelector('.tabheader__items'); //функция скрытия табов  и класса выделения текста
...
function timer() {
  ...
  function getTimerRemaining(endtime) {
    ...
    let hours = Math.floor(t / (60 * 60 * 1000) % 24); // console.log(hours);

    let minutes = Math.floor(t / (60 * 1000) % 60); // let minutes2 = Math.floor((t / 60 / 1000) % 60); // или так
    // console.log(minutes);

    let seconds = Math.floor(t / 1000 % 60); // console.log(seconds);
  ...
  function setClock(selector, endtime) {
    ...
    const timer = document.querySelector(selector); //теперь уже находим идентификаторы внутри класса timer

    const days = timer.querySelector('#days');
    const hours = timer.querySelector('#hours');
    const minutes = timer.querySelector('#minutes');
    const seconds = timer.querySelector('#seconds');
    let timeInterval = setInterval(updateClock, 1000); //внутри ф. создаем ф. которая будет обновлять данные часов
    ...
    function updateClock() {
      ...
      seconds.innerHTML = getZero(t.seconds); //следим когда время выйдет
...
(__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill)(); // синтаксис Common JS можно смешать с модулями
